...
| --- | --- |
| Out-of-order safety | The fold sorts by rank, so a late `delivered` after a `read` cannot move state backward - and the `delivered` timestamp is still captured. |
...
3. `SELECT ... FOR UPDATE` row lock. For a multi-instance deployment (several CRM replicas behind Postgres + Redis), the row lock serializes writers across processes, where the in-process lock cannot reach. It is a no-op on SQLite, which the keyed lock covers.

The append-only log means even if a projection row ever did go stale, the data is not lost - re-folding the events reconstructs the correct state. The cache is an optimization, not the source of truth.
...
- Channel client - the outbound HTTP call to the channel. A fake is injected in the integration test to force the retry and dead-letter path.
...
The co-pilot turns a natural-language goal into a small typed segment DSL: a closed set of fields (`total_orders`, `total_spend`, `days_since_last_order`, `lifecycle_stage`, `channel_optin`, `city`, and more) and a validated set of operators. The DSL is parsed at a single trust boundary and compiled to parameterized SQL.
