...
```bash
cp .env.example .env        # optional: add your Gemini API key for the AI co-pilot
...
The CRM boots against a local SQLite file with an in-process bus, so it runs with no external dependencies at all. Set `CRM_GEMINI_API_KEY` to use the Gemini-powered AI planner; without it, a deterministic local planner keeps the co-pilot fully functional offline.
...
- **AI:** Google Gemini 2.5 Flash (strict JSON-schema output) with a deterministic local planner fallback.
